(function () {
  ...
  const createBookElement = (book, index) => {
    ...
    // Prepare Cover URL
    const coverUrl = metrics.coverPath ? `url("${metrics.coverPath}")` : "";
    if (metrics.coverPath) {
      el.classList.add("has-cover");
      el.style.setProperty("--cover-image", coverUrl);
    } else {
    ...
    const stackHtml = `
        <div class="stack-content">
            <div class="stack-cover" style='${coverUrl ? `background-image: ${coverUrl}` : ''}'></div>
